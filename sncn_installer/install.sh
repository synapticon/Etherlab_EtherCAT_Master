...
do_configure() {
  cd $SCRIPT_DIR/..
  ./bootstrap
  ./configure --enable-sii-assign --disable-8139too --enable-hrtimer --enable-cycles --prefix=$ETHERCAT_INSTALL_PREFIX
...
do_setup_interfaces () {
  ...
  sudo sed -i '/MASTER.*DEVICE/d' /etc/sysconfig/ethercat
  iface_index="0"
  ...
  if [[ -n $(lscpu | sed '13q;d' | grep "Intel(R) Pentium(R) CPU N4200 @ 1.10GHz") ]];then
    ...
    echo "MASTER${iface_index}_DEVICE=\"$MAC\"" | sudo tee --append $ETHERCAT_SYSCONFIG
  else
  # Standard laptop
